...
class ProductoCtrl{
  ...
  static Future<bool> insertProductoBar2(String id_bar, String id_prod, double precio) async {
    http.Response response = await http.post(
  ...
  static Future<bool> insertProductoBar(String id_bar, String id_prod, double precio, File img) async {
    print(img);
    try {
      img.existsSync();
      final image = await MultipartFile.fromFile(img.path, filename: basename(img.path));
      ...
      final response = await Dio().post(GlobalVars.apiUrl + "lista_productos", data: formData, options: Options(
