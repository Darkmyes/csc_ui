...
class EstiloVidaCtrl{
  ...
  static Future<bool> actualizarEstiloVida(int id_estilo, String estilo) async {
    http.Response response = await http.put(
      GlobalVars.apiUrl + "estilos_vida/"+id_estilo.toString(),
      headers: <String, String>{
        'Content-Type': 'application/json',
      },
      body: jsonEncode(<String, dynamic>{
  ...
  static Future<bool> eliminarEstiloVida(int idEnfermedad) async {
    http.Response response = await http.delete(
      GlobalVars.apiUrl + "estilos_vida/"+idEnfermedad.toString()
    ); 
    if (response.statusCode == 200) {
      return true;
    } else {
